/* =========================================================
   ...
   HTTP server murni (node:http) + REST API + static file server.
   ========================================================= */
   ...
   async function handleApi(req, res, pathname) {
     ...
     try {
       ...
       if (pathname === "/api/auth/login" && method === "POST") {
         ...
         // Validasi password teks biasa langsung (tanpa verifikasi hash/salt)
   ...
   (async () => {
     try {
       ...
       await DB.init();
       console.log("Database siap (nama database: sirela_db, cek di phpMyAdmin).");
       server.listen(PORT, () => {
         console.log(`SIRELA server jalan di http://localhost:${PORT}`);
       });
     } catch (err) {
       console.error("Gagal konek ke database MySQL/MariaDB:");
